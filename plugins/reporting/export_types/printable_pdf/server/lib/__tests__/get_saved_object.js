...
// we want to use the `url` class to inspect the properties of the urlHash
// but to do so we have to resolve it using a fake base url
const parseUrlHashUsingFakeBase = (urlHash) => {
  return url.parse(url.resolve('http://localhost:8080', `#${urlHash}`));
};
  ...
  objectTypes.forEach(function (objectType) {
    describe(`type ${objectType}`, function () {
      ...
      it('should contain specific props', async function () {
        ...
        expect(savedObject).to.have.property('urlHash');

        expect(savedObject.searchSource).to.be.an(Object);
        expect(savedObject.urlHash).to.be.a('string');
      });
      ...
      describe('urlHash', function () {
        ...
        it('should provide app url', async function () {
          ...
          const params = parseUrlHashUsingFakeBase(savedObject.urlHash);
        ...
        it('should remove the refreshInterval value', async function () {
          const savedObject = await getSavedObject(request, objectType, mockObject._id, query);
          const params = parseUrlHashUsingFakeBase(savedObject.urlHash);
          expect(params).to.not.contain('refreshInterval');
        });

        it('should always convert to absolute time', async function () {
          const savedObject = await getSavedObject(request, objectType, mockObject._id, query);
          const params = parseUrlHashUsingFakeBase(savedObject.urlHash);
