...
describe('map nodes metrics', () => {
  it('summarizes the info', () => {
    ...
    expect(mapNodesMetrics(metricsForNodes, nodesInfo, timeOptions)).to.eql({
      'sg-ldUAbRUCOPIytxxhcIA': {
        node_cpu_utilization: {
          metric: {
            app: 'elasticsearch',
          ...
          summary: { minVal: 0, maxVal: 0, lastVal: 0, slope: -1 }
        },
        node_jvm_mem_percent: {
          metric: {
            app: 'elasticsearch',
          ...
          summary: { minVal: 13, maxVal: 35, lastVal: 13, slope: -1 }
        }
      }
    });
  });
