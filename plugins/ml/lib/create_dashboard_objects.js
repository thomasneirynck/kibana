...
/*
 * Adds the pre-built Ml dashboard objects (searches, visualizations, dashboards)
  ...
  function createObject(id, type, source) {
    callWithInternalUser('create', {
      ...
      if (error) {
        ...
        console.log('Error creating Ml object:', error);
      }
    });
  }

  _.each(searches, (obj) => {
    plugin.status.yellow('Creating Ml searches');
