...
module.controller('MlNewJobSingleStep1', function ($scope, $route, timefilter) {
  ...
  $scope.makeUrl = (pattern) => {
    if (!pattern) return;
    return '#/jobs/new_job/simple/single_metric/create?index=' + encodeURIComponent(pattern.id);
  };
});
