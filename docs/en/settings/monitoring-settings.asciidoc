...
how monitoring data is collected from {kib} and displayed in {kib} by
configuring settings in the `kibana.yml` file. There are also
...
Set to `false` to disable {monitoring} in {kib}. Unlike the
