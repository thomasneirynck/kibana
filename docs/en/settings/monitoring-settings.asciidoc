...
You configure `xpack.monitoring.collection` settings in `elasticsearch.yml` to
control how data is collected from your Elasticsearch nodes. For more
information, see {xpack-ref}/monitoring-settings.html[Monitoring Settings].
//TO-DO: Update URL when content moves to Elasticsearch Reference.

You can adjust how monitoring data is displayed in the
Monitoring UI by configuring <<monitoring-ui-settings, `xpack.monitoring`>>
settings in `kibana.yml`.

For more information, see
{xpack-ref}/configuring-monitoring.html[Configuring Monitoring].
...
{es-ref}/search-aggregations-bucket-terms-aggregation.html#_size[Terms Aggregation].
Defaults to 10000.
