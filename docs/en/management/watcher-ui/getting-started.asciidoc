...
Watcher is automatically enabled when you {xpack-ref}/installing-xpack.html[install X-Pack] into Elasticsearch and Kibana.
...
To delete or edit a watch, click on its checkbox in the list and then click the Delete or Edit button. Watches that do not have a checkbox are <<cluster-alerts,cluster alerts>> created by X-Pack monitoring and are immutable.
