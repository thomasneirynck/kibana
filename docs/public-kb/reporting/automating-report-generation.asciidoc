...
You request reports through three {reporting} endpoints:
...
To configure a watch to email reports, you use the
<<configuring-email-actions,`reporting` attachment type>> in an `email` action.
