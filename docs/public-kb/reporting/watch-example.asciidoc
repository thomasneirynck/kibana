...
<1> You must <<configuring-email,configure at least one email account>> to enable Watcher to send email.
...
<5> Provide user credentials for a user with permission to access Kibana and {reporting}. For more information, see <<reporting-app-users, Setting up a Reporting Role>>.
